...
[@helpers.html]
  ...
  [@helpers.body]
    ...
    [@helpers.main]
      <form action="${request.contextPath}/oauth2/register" method="POST">
                ...
                <fieldset class="flex flex-col gap-2 md:gap-4">
                  ...
                  [#else]
                    [#if application.registrationConfiguration.loginIdType == 'email']
                        <div>
                          [@helpers.input label="Email" type="text" name="user.email" id="email" autocomplete="username" autocapitalize="none" autocorrect="off" spellcheck="false" autofocus=true placeholder=theme.message('email')  required=true /]
                        </div>
                    [#else]
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 md:gap-2">
                      [@helpers.input label="Password" type="password" name="user.password" id="password" autocomplete="new-password" placeholder=theme.message('password') required=true class="flex-1 shrink gap-2 self-stretch px-3.5 py-3 w-full rounded  max-md:max-w-full p-2${(errorMessages?size > 0)?then(' outline-red-500 outline-2', '')}" /]
                      [#if application.registrationConfiguration.confirmPassword]
                        [@helpers.input label="Confirm Password" type="password" name="passwordConfirm" id="passwordConfirm" autocomplete="new-password" placeholder=theme.message('passwordConfirm') required=true/]
                      [/#if]
                    </div>
